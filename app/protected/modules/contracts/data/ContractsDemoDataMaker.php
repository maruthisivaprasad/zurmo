<?php
    ...
    /**
     * Class that builds demo Contracts.
     */
    class ContractsDemoDataMaker extends DemoDataMaker
    {
        ...
        public function makeAll(& $demoDataHelper)
        {
            ...
            for ($i = 0; $i < $this->resolveQuantityToLoad(); $i++)
            {
                ...
                $contract->contacts->add($demoDataHelper->getRandomByModelName('Contact'));
                $contract->account      = $contract->contacts[0]->account;
                $contract->owner        = $contract->contacts[0]->owner;
                $currencyValue             = new CurrencyValue();
                $currencyValue->currency   = $currencies[array_rand($currencies)];
                $contract->amount       = $currencyValue;
                $this->populateModel($contract);
                $saved = $contract->save();
                assert('$saved');
                $contracts[]           = $contract->id;
            }
            $demoDataHelper->setRangeByModelName('Contract', $contracts[0], $contracts[count($contracts)-1]);
        }
        ...
        public function populateModel(& $model)
        {
            ...
            $ContractRandomData = ZurmoRandomDataUtil::
                                     getRandomDataByModuleAndModelClassNames('ContractsModule', 'Contract');

            parent::populateModel($model);
            $name        = RandomDataUtil::getRandomValueFromArray($contractRandomData['names']);
            $model->name = $name;
