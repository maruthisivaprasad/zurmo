<?php
     ...
     */
    class ContractSuperUserExportWalkthroughTest extends ZurmoWalkthroughBaseTest
    {
        ...
        public function testDownloadDefaultControllerActions()
        {
            ...
            for ($i = 0; $i < 2; $i++)
            {
                $contracts[] = contractrtunityTestHelper::createContractWithAccountByNameForOwner('superOpp' . $i, $super, $account);
            }
            ...
            $this->setGetArray(array(
                ...
                'ajax'           => '',
                'selectAll' => '',
                'selectedIds' => "{$contracts[0]->id}, {$contracts[1]->id}")
            );
            $response = $this->runControllerWithExitExceptionAndGetContent('contracts/default/export');
            $this->assertEquals('Testing download.', $response);
        ...
        */
        public function testAsynchronousDownloadDefaultControllerActions()
        {
            ...
            foreach ($contracts as $contract)
            {
                $selectedIds .= $contract->id . ","; // Not Coding Standard
            }
            $this->setGetArray(array(
                'ContractsSearchForm' => array(
                    'anyMixedAttributesScope' => array(0 => 'All'),
